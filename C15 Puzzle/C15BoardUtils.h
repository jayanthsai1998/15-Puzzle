...
Board * createBoard(int rows, int cols)
{
	...
	int k;
	int **mytable = (int**)malloc(rows*sizeof(int*));
	for (k = 0; k < rows; k++)
	{
		mytable[k] = (int*)malloc(cols*sizeof(int));
	}
	changeThis->table = mytable;
...
{
	int i, j, p = 0;
	if (rows <= 3 || rows >= 5 || cols <= 3 || cols >= 5 || source == NULL || tobeCopiedArray == NULL)
	{
	}
	else
	{
...
int checkBoard(Board * board)
{
	...
	for (p = 0; p < 4; p++)
	{
		for (q = 0; q < 3; q++)
		{
			if (p != 3 && q != 2)
			{
				if ((board->table[p][q]) >(board->table[p][q + 1]))
				{
					return 0;
				}
			}
		...
		if (p < 3)
		{
			if ((board->table[p][q]) >(board->table[p + 1][0]))
			{
				return 0;
			}
		}
